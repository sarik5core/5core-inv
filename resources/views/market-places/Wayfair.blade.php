@extends('layouts.vertical', ['title' => 'Wayfair', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
                    ...
                    <div id="percent-edit-div" class="d-flex align-items-center">
                        <div class="input-group" style="width: 150px;">
                            <input type="number" id="updateAllSkusPercent" class="form-control" min="0"
                                max="100" value="{{ $wayfairPercentage }}" step="0.01" title="Percent" disabled />
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Wayfair Product Analysis</h4>
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-3 align-items-center justify-content-between">
                        <div class="d-flex flex-wrap gap-2 align-items-center">
                            ...
                            <!-- W Dil% Filter -->
                            <div class="dropdown manual-dropdown-container ">
                                <button class="btn btn-light dropdown-toggle" type="button" id="wDilFilterDropdown">
                                    <span class="status-circle default"></span> W Dil%
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="wDilFilterDropdown">
                                    <li><a class="dropdown-item column-filter" href="#" data-column="W Dil%"
                                            data-color="all">
                                            <span class="status-circle default"></span> All W Dil</a></li>
                                    <li><a class="dropdown-item column-filter" href="#" data-column="W Dil%"
                                            data-color="red">
                                            <span class="status-circle red"></span> Red</a></li>
                                    <li><a class="dropdown-item column-filter" href="#" data-column="W Dil%"
                        ...
                        <div class="d-flex flex-wrap gap-2 align-items-center">
                            <!-- Export Button -->
                            <a href="{{ route('wayfair.analytics.export') }}" class="btn btn-success">
                                <i class="fas fa-file-export me-1"></i> Export Live/Listings
                            </a>

                            <!-- Import Button -->
                            <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                data-bs-target="#wayfairImportModal">
                                <i class="fas fa-file-import me-1"></i> Import Live/Listings
                            </button>
                        </div>
                    </div>

                    <div class="modal fade" id="wayfairImportModal" tabindex="-1"
                        aria-labelledby="wayfairImportModalLabel" aria-hidden="true">
                        <div class="modal-dialog">
                            <form action="{{ route('wayfair.analytics.import') }}" method="POST"
                                enctype="multipart/form-data" class="modal-content" id="wayfairImportForm">
                                @csrf
                                <div class="modal-header">
                                    <h5 class="modal-title" id="wayfairImportModalLabel">Import Wayfair Data</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                        aria-label="Close"></button>
                                </div>
                                <div class="modal-body">
                                    <!-- File Input -->
                                    <div class="mb-3">
                                        <label for="wayfairExcelFile" class="form-label">Select Excel File</label>
                                        <input type="file" class="form-control" id="wayfairExcelFile"
                                    ...
                                    <div class="alert alert-info">
                                        <small>
                                            ...
                                            Download the <a href="{{ route('wayfair.analytics.sample') }}"
                    ...
                    <div class="btn-group time-navigation-group" role="group" aria-label="Parent navigation">
                        <button id="play-backward" class="btn btn-light rounded-circle" title="Previous parent" disabled>
                        ...
                        <button id="play-auto" class="btn btn-light rounded-circle" title="Show all products" disabled>
                            <i class="fas fa-play"></i>
                        </button>
                        <button id="play-forward" class="btn btn-light rounded-circle" title="Next parent" disabled>
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <div class="form-inline">
                            <div class="form-group mr-2">
                                ...
                                <select id="row-data-type" class="form-control form-control-sm" disabled>
                    ...
                    <div id="data-loader" class="card-loader-overlay" style="display: none;">
                        <div class="loader-content">
                            ...
                            <div class="loader-text">Loading Wayfair data...</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on("change", ".sPriceInput", function() {
                ...
                $.ajax({
                    url: "/update-wayfair-sku-pricing",
                    type: "POST",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        "sku": sku,
                    ...
                    success: function(info) {
                        alert("Request Sent to Wayfair, Pls Wait to Reflect Everywhere");
                    }
                });
        ...
        $(document).ready(function() {
            ...
            function updateSoldCounts() {
                ...
                filteredData.forEach(item => {
                    if (!item.is_parent) {
                        const l30 = parseFloat(item['W L30']) || 0;
            ...
            function renderTable() {
                ...
                filteredData.forEach(item => {
                    ...
                    if (item.is_parent) {
                        ...
                        const buyerLink = item.raw_data['Blink'] || '';
                        const sellerLink = item.raw_data['Seller'] || '';
                    ...
                    $row.append($('<td>').text(item['W L30']));

                    // // A DIL with color coding
                    $row.append($('<td>').html(
                        `<span class="dil-percent-value ${getDilColor(item['W Dil%'])}">${!isNaN(item['W Dil%']) ? Math.round(item['W Dil%'] * 100) + '%' : '0%'}</span>`
                    ));
                ...
                //            data-item='${JSON.stringify(item.raw_data)}'>V</span>`
                    ...
                    // //price with tooltip
                    $row.append($('<td>').html(
                        `$${(Number(item['Wayfair Price']) || 0).toFixed(2)}
                    ...
                    $row.append($('<td>').html(
                        `<div style="display:flex;align-items:center">
                            <span class="sPriceText" data-sku="${item['(Child) sku']}" style="min-width:100px; display:inline-block;">
                                ` + item['Wayfair Price'] + `
                            </span>
                            <input 
                                value="` + item['Wayfair Price'] + `" 
                                data-sku="` + item['(Child) sku'] + `" 
                ...
                //         data-item='${JSON.stringify(item.raw_data)}'>P</span>`
                    ...
                    // // TPFT
                    // $row.append($('<td>').html(
                    //     typeof item.TPFT === 'number' && !isNaN(item.TPFT) ?
                    //     `<span style="position: relative; padding-left: 14px; text-align: left;">${(item.TPFT * 100).toFixed(2)}%</span>` :
                    //     ''
                    // ));
            ...
            $(document).on('change', '.listed-checkbox, .live-checkbox', function() {
                ...
                $.ajax({
                    url: '/wayfair/update-listed-live',
                    method: 'POST',
                    data: {
                        sku: sku,
                        field: field,
            ...
            window.openModal = function(selectedItem, type) {
                try {
                    ...
                    const cachedData = wayfairDataCache.get(itemId);
                    const dataToUse = cachedData || itemData;

                    // Store the data in cache if it wasn't already
                    if (!cachedData) {
                        wayfairDataCache.set(itemId, itemData);
                    }
                    ...
                    switch (type.toLowerCase()) {
                        ...
                        case 'price view':
                            fieldsToDisplay = [{
                                ...
                                {
                                    title: 'Wayfair Price',
                                    content: selectedItem['Wayfair Price']
                                },
                                {
                                    title: 'PFT%',
            ...
            function saveChanges(contentElement, title, slNo, isHyperlink, updatedValue, isCheckbox, rowElement) {
                ...
                wayfairDataCache.updateField(itemId, title, cacheUpdateValue);
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        ...
                        // Revert cache
                        wayfairDataCache.updateField(itemId, title, originalValue);
            ...
            function calculateTotals() {
                try {
                    ...
                    filteredData.forEach(item => {
                        ...
                        metrics.wl30Total += parseFloat(item['W L30']) || 0;
                        metrics.wDilTotal += parseFloat(item['W Dil%']) || 0;
                    ...
                    $('#wl30-total').text(metrics.wl30Total.toLocaleString());
                    $('#wDil-total').text(Math.round(metrics.wDilTotal / divisor * 100) + '%');

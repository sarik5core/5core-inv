@extends('layouts.vertical', ['title' => 'eBay Low View', 'mode' => $mode ?? '', 'demo' => $demo ?? ''])
...
@section('content')
    @include('layouts.shared/page-title', ['page_title' => 'eBay Low View', 'sub_title' => 'eBay View'])

    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">eBay Low Analysis</h4>
                    ...
                    <div class="table-container">
                        <table class="custom-resizable-table" id="ebayLow-table">
                            <thead>
                                <tr>
                    ...
                    <div id="data-loader" class="card-loader-overlay" style="display: none;">
                        <div class="loader-content">
                            ...
                            <div class="loader-text">Loading eBay Low data...</div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function loadData() {
                showLoader();
                return $.ajax({
                    url: '/ebay/low-visibility/view-data',
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
            ...
            function renderTable() {
                const $tbody = $('#ebayLow-table tbody');
                ...
                const $headers = $('#ebayLow-table thead th');
                ...
                filteredData.forEach(item => {
                    ...
                    $row.append($('<td>').html(`
                        <div class="sku-tooltip-container">
                            <span class="sku-text">${item['E L30']}</span>
                            <div class="sku-tooltip">
                                <div class="sku-link"><strong>L60:</strong> </div>
                    ...
                    $row.append($('<td>').html(`
                        ...
                        data-slno="${item['Sl']}" data-type="reason"></i>
                    `));
                    ...
                    $row.append($('<td>').html(`
                        ...
                        <i class="fas fa-plus reason-action-plus" style="cursor:pointer; color:#007bff; margin-left:8px;" 
                        data-slno="${item['Sl']}" data-type="action_required"></i>
                    `));
                    ...
                    $row.append($('<td>').html(`
                        ...
                        <i class="fas fa-plus reason-action-plus" style="cursor:pointer; color:#007bff; margin-left:8px;" 
                        data-slno="${item['Sl']}" data-type="action_taken"></i>
                    `));
            ...
            function saveChanges(contentElement, title, slNo, isHyperlink, updatedValue, isCheckbox, rowElement) {
                ...
                if (index !== -1) {
                    ...
                    if (title === 'sprice' && filteredData[index].raw_data) {
                        ...
                        ebayDataCache.updateField(itemId, 'Spft%', Spft);
            ...
            // Initialize column toggle functionality with persistence
            function initColumnToggle() {
                ...
                // Load saved hidden columns from localStorage
                let hiddenColumns = JSON.parse(localStorage.getItem('lowHiddenColumns')) || [];
                ...
                $headers.each(function() {
                    ...
                    const isChecked = !hiddenColumns.includes(field);
                    ...
                    // Apply initial visibility from storage
                ...
                $menu.on('change', '.column-toggle-checkbox', function() {
                    ...
                    // Update storage
                    if (!isVisible) {
                        if (!hiddenColumns.includes(field)) hiddenColumns.push(field);
                    } else {
                        hiddenColumns = hiddenColumns.filter(f => f !== field);
                    }
                    localStorage.setItem('lowHiddenColumns', JSON.stringify(hiddenColumns));
                });

                $('#showAllColumns').on('click', function() {
                    $menu.find('.column-toggle-checkbox').prop('checked', true).trigger('change');
                    hiddenColumns = [];
                    localStorage.removeItem('lowHiddenColumns');
                    $menu.removeClass('show');
                });
            }
            ...
            $('#saveReasonActionBtn').on('click', function() {
                ...
                [filteredData, tableData].forEach(arr => {
                    const row = arr.find(item => item['Sl'] == slNo);
                    if (row) {
                ...
                if (sku) {
                    $.ajax({
                        url: '/ebay-low-visibility/reason-action/update',
                        method: 'POST',
                        data: {
                            sku: sku,
                            reason: reason,

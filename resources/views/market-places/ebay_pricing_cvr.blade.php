...
@section('css')
    ...
    <style>
        ...
        #skuDetailsModal tbody tr:nth-child(odd) {
            background: #f9fafb;
        }

        #skuDetailsModal tbody tr:hover {
            background: ;
            transition: background 0.2s ease;
        }

        #skuDetailsModal td a {
            color: #0d6efd;
            text-decoration: underline;
            word-break: break-all;
        }

        #skuDetailsModal td a:hover {
            color: #0a58ca;
        }
                    ...
                    <div class="table-container">
                        <table class="custom-resizable-table" id="ebay-table">
                            <thead>
                                <tr>
                                    <th data-field="sl_no">
                                        <div class="sortable-header">SL No. <span class="sort-arrow">↓</span></div>
                                    </th>
                                    ...
                                    <th data-field="(Child) sku" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center">
                                            <div class="d-flex align-items-center sortable-header">
                                    ...
                                    <th data-field="fba">
                                        <div class="sortable-header">FBA <span class="sort-arrow">↓</span></div>
                                    </th>
                                    ...
                                    <th data-field="E L30" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center" style="gap: 4px">
                                            <div class="d-flex align-items-center sortable-header">
                                    ...
                                    <th data-field="E Dil%" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center" style="gap: 4px">
                                            <div class="d-flex align-items-center sortable-header">
                                    ...
                                    <th>NRL</th>
                                    <th>Hide</th>
                                    <th data-field="OV CLICKS L30" style="vertical-align: middle; white-space: nowrap;">
                                        <div class="d-flex flex-column align-items-center" style="gap: 4px">
                                            <div class="d-flex align-items-center sortable-header">
...
@section('script')
    ...
    <script>
        $(document).on('change', '.fba-checkbox', function() {
            let shopifyId = $(this).data('id');
            let fbaStatus = $(this).is(':checked') ? 1 : 0;

            $.ajax({
                url: '/update-fba-status-ebay',
                method: 'POST',
                data: {
                ...
                success: function(response) {
                    console.log('FBA status updated:', response);
                },
                error: function(xhr) {
                    console.error('Error updating FBA status:', xhr.responseText);
                }
            });
        });
        flatpickr("#duration", {
        ...
        $(document).ready(function() {
                        ...
                        if (response && response.data) {
                            ...
                            tableData = validData.map((item, index) => {
                                ...
                                if (Number(item['OV CLICKS L30']) > 0) {
                                    scvr = Number(item['E L30']) / Number(item[
                                        'OV CLICKS L30']);
                                }
                            ...
                            console.log('All Data Loaded:', tableData);
                            filteredData = [...tableData];
                        }
            ...
            function renderTable() {
                ...
                filteredData.forEach(item => {
                    ...
                    // if (item['INV'] === 0 || item['OV CLICKS L30'] === 0) {
                    //     return;
                    // }
                    if (item['INV'] === 0) {
                        return;
                    }
                    if (item['OV CLICKS L30'] < 49) {
                        return;
                    }
                    ...
                    $row.append(
                        $('<td>').append(
                            $('<input>', {
                                type: 'checkbox',
                                checked: parseInt(item.fba) === 1,
                                class: 'fba-checkbox',
                                'data-id': item['(Child) sku'] || ''
                            })
                        )
                    );
                    ...
                    if (item.is_parent) {
                        ...
                        const $hideCheckbox = $('<input type="checkbox" class="hide-checkbox" />')
                            .prop('checked', item['Hide'] === true || item['Hide'] === 'true' || item[
                                'Hide'] === '1')
                            .data('original-value', item['Hide']);
                        $hideContainer.append($hideCheckbox);
                    ...
                    let scvrValue = 0;
                    if (Number(item['OV CLICKS L30']) > 0) {
                        scvrValue = (Number(item['E L30']) / Number(item['OV CLICKS L30'])) * 100;
                    }
                    $row.append($('<td>').html(
                    ...
                    $row.append($('<td>').html(`
                    <span class="tooltip-container" style="margin-left:8px">
                        <i class="fas fa-eye text-info eye-view-trigger"
    ...
    ` : ''
            ...
            function initCheckBoxEditHandlers() {
                ...
                $(document).off('change', '.nr-checkbox, .hide-checkbox');
                $(document).on('change', '.nr-checkbox, .hide-checkbox', function() {
                    const $checkbox = $(this);
                    const $row = $checkbox.closest('tr');
                    const slNo = $row.find('td:eq(0)').text();
                    const rowData = filteredData.find(item => item['Sl'] == slNo || item['SL No.'] == slNo);
                    const sku = rowData ? rowData['(Child) sku'] : null;
                    const isNR = $checkbox.hasClass('nr-checkbox');
                            ...
                            fieldsToDisplay = [{
                                ...
                                },
                                {
                                    ...
                                    content: (Number(selectedItem['OV CLICKS L30']) > 0) ?
                                        ((Number(selectedItem['E L30']) / Number(selectedItem[
    ...
    `);
            ...
            function setupEditHandlers(modalId) {
                ...
                const editableFields = ['sprice', 'LMP 1', 'link 1', 'lmp 2', 'link 2', 'lmp 3', 'link 3',
            ...
            function saveChanges(contentElement, title, slNo, isHyperlink, updatedValue, isCheckbox, rowElement) {
                ...
                if (index !== -1) {
                    ...
                    if (title === 'sprice' && filteredData[index].raw_data) {
                        ...
                        let Spft = 0;
                        if (SPRICE !== 0) {
                            Spft = (SPRICE * 0.74 - LP - SHIP) / SPRICE;
                        }
            ...
            function initSorting() {
                // A list of all fields that should be sorted numerically.
                const numericFields = [
                    'sl_no', 'fba', 'INV', 'L30', 'ov_dil', 'E L30', 'E Dil%',
                    'OV CLICKS L30', 'SCVR', 'eBay Price', 'PFT %', 'Roi',
                    'SPRICE', 'SPFT', 'SROI'
                ];

                // Attach the click event handler to the .sortable-header div
                $('#ebay-table thead').on('click', '.sortable-header', function(e) {
                    if (isResizing) {
                        e.stopPropagation();
                        return;
                    }
                    ...
                    if (!dataField) {
                        return; // Exit if the header has no data-field
                    }
                    ...
                    // Update UI arrows
                    $('.sort-arrow').html('↓'); // Reset all arrows
                    ...
                    filteredData.sort((a, b) => {
                        ...
                        // Check if the current field should be sorted as a number
                        if (numericFields.includes(dataField)) {
                            // Use parseFloat to handle numbers, default to 0 if null/undefined
                            return (parseFloat(valA || 0) - parseFloat(valB || 0)) * currentSort
                                .direction;
                        }

                        // Otherwise, perform a case-insensitive string comparison
                        return String(valA || '').localeCompare(String(valB || '')) * currentSort
                            .direction;
                    });
            ...
            function applyColumnFilters() {
                ...
                if (el30Filter && el30Filter !== 'all') {
                    filteredData = filteredData.filter(item => {
                        const el30 = Number(item['E L30']) || 0;
            ...
            function getColorForColumn(column, rowData) {
                ...
                if (column === 'SCVR') {
                    ...
                    if (Number(rowData['OV CLICKS L30']) > 0) {
                        scvr = Number(rowData['E L30']) / Number(rowData['OV CLICKS L30']);
                    }
                ...
                try {
                    ...
                    filteredData.forEach(item => {
                        ...
                        metrics.scvrSum += (Number(item['OV CLICKS L30']) > 0) ?
                            (Number(item['E L30']) / Number(item['OV CLICKS L30'])) :
                            0;
                        metrics.rowCount++;
                    });
            ...
            $(document).on('click', '.eye-view-trigger', function() {
                const rawData = $(this).data('item');
                let parsedData;
                ...
                // tbody साफ करो
                $('#skuDetailsContent').empty();

                // हर key–value को row में डालो
                Object.entries(parsedData).forEach(([key, value]) => {
                    ...
                    // Links handle
                    else if (typeof value === 'string' && (value.startsWith('http://') || value
                            .startsWith('https://'))) {
                        displayValue =
                            `<a href="${value}" target="_blank" rel="noopener noreferrer">${value}</a>`;
                    }

                    $('#skuDetailsContent').append(`
            <tr>
    ...
    <div class="modal fade" id="skuDetailsModal" tabindex="-1" aria-hidden="true">
                ...
                <div class="modal-body p-0">
                    <table class="table table-bordered table-striped mb-0">
                        <thead class="table-light">
                            <tr>
                                <th style="width: 30%; color: #000000;">Field</th>
                                <th class="p-3" style="color: #000000;">Value</th>
                            </tr>
                        </thead>
                        <tbody id="skuDetailsContent" style="padding: 0%"></tbody>
                    </table>
                </div>
            </div>
        </div>

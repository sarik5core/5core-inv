...
@foreach ($data as $row)
    ...
    <tr style="{{ $isParent ? 'background-color: #e0f7ff;' : '' }}" data-is-parent="{{ $row->is_parent ? '1' : '0' }}">
        ...
        <td>
            ...
            @else
                <input type="number"
                   class="form-control form-control-sm order-qty"
                   placeholder="Enter Approved QTY"
                   data-sku="{{ $row->SKU }}"
                   data-column="approved_qty"
                   value="{{ $row->{'Approved QTY'} ?? '' }}"
                   min="0"
                   max="99999"
                   style="background:#f8fafd;color:#000;width:80px;">
        ...
        <td>
            <span class="fw-bold text-dark" style="font-size: 0.97em;">
            @php
                $dateOfAppr = $row->{'Date of Appr'} ?? null;
            @endphp
            {{ $dateOfAppr ? \Carbon\Carbon::parse($dateOfAppr)->format('d-m-y') : '-' }}
            </span>
        </td>
        <td>
        ...
        <td>
            ...
            <button class="btn btn-sm btn-outline-success open-review-modal"
                ...
                data-improvement="{{ $review->improvement ?? '' }}"
                data-clink="{{ $row->Clink ?? '' }}"
        ...
        @if(!empty($row->{'RFQ Form Link'}))
            <td>
                <a href="{{ $row->{'RFQ Form Link'} }}" class="copy-btn btn btn-sm btn-outline-primary" data-link="{{ $row->{'RFQ Form Link'} }}">
        ...
        @if(!empty($row->{'Rfq Report Link'}))
            <td>
                <a href="{{ $row->{'Rfq Report Link'} }}" class="copy-btn btn btn-sm btn-outline-success" data-link="{{ $row->{'Rfq Report Link'} }}">
                    <i class="mdi mdi-link"></i> Open
                </a>
            </td>
        @else
            <td contenteditable="true" class="editable-cell report-cell" data-placeholder="Enter Rfq Report Link"
                ...
                data-original-value="{{ $row->{'Rfq Report Link'} ?? '' }}"
                style="background:#f8fafd;color:#888;"></td>
        @endif

        @if(!empty($row->sheet_link))
            <td>
                <a href="{{ $row->sheet_link }}" class="copy-btn btn btn-sm btn-outline-success" data-link="{{ $row->sheet_link }}">
                    <i class="mdi mdi-link"></i> Open
                </a>
            </td>
        @else
            <td contenteditable="true" class="editable-cell report-cell" data-placeholder="Enter Sheet Link"
                ...
                data-original-value="{{ $row->sheet_link ?? '' }}"
                style="background:#f8fafd;color:#888;">
            </td>
        @endif
        ...
        <td>
            @if($isParent)
                <input type="number"
                   ...
                   value="{{ $row->order_qty ?? '' }}"
                   min="0"
                   style="background:#f8fafd;color:#000;width:110px;"
                   readonly>
            @else
                <input type="number"
                   class="form-control form-control-sm order-qty"

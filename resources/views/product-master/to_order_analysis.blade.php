...
@endsection
@section('content')
@include('layouts.shared.page-title', ['page_title' => 'To Order Analysis', 'sub_title' => 'TO BE DC'])
                ...
                <div class="row mb-4 g-3 align-items-end justify-content-between">
                    ...
                    <div class="col-auto">
                        ...
                        <div class="fw-bold text-primary" style="font-size: 1.1rem;">
                            {{ collect($allProcessedData)->filter(fn($row) =>
                                strtolower($row->Stage) !== 'mfrg progress' &&
                                !$row->is_parent &&
                                (int) $row->{'Approved QTY'} > 0
                            )->count() }}
                        </div>
                    </div>
                    ...
                    <div class="col-auto">
                        ...
                        <input type="text" id="search-input" class="form-control form-control-sm" placeholder="Search suppliers...">
                    </div>
                </div>

                <div class="table-responsive" id="table-container">
                    <table class="table table-centered table-bordered table-responsive mb-0" id="suppliers-table">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="review_parent" name="parent" readonly>
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="review_supplier" name="supplier" readonly>
                        </div>

                        <div class="col-md-12">
                                <label for="positive_review" class="form-label">✨ Positive Review <span class="text-danger">*</span></label>
                        ...
                        <div class="col-md-12">
                                <label for="negative_review" class="form-label">⚠️ Negative Review <span class="text-danger">*</span></label>
                        ...
                        <div class="col-md-12">
                                <label for="improvement" class="form-label">📈 Improvement Required <span class="text-danger">*</span></label>
                        ...
                        <div class="row mt-3">
                            <div class="col-md-6">
                                <label for="date_updated" class="form-label">📅 Date Updated <span class="text-danger">*</span></label>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <a href="#" class="btn btn-outline-primary" id="clink" target="_blank">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('stage-filter').addEventListener('change', function () {
        ...
        rows.forEach(row => {
            ...
            row.style.display = !selectedStage || stageSelect.value.toLowerCase() === selectedStage ? '' : 'none';
        });
    ...
    function attachEditableListeners() {
        document.querySelectorAll('.editable-cell').forEach(cell => {
            cell.addEventListener('blur', function () {
                const sku = this.dataset.sku, column = this.dataset.column, value = this.innerText.trim();
                if (!value) return;
                ...
                .then(res => {
                    if (res.success) {
                        const btnClass = column === 'RFQ Form Link' ? 'btn-outline-primary' : 'btn-outline-success';
                        this.outerHTML = `<td><a href="${value}" class="btn btn-sm ${btnClass}" data-link="${value}"><i class="mdi mdi-link"></i> Open</a></td>`;
                    } else {
        ...
        document.querySelectorAll('.order-qty').forEach(input => {
            input.addEventListener('blur', function () {
                const sku = this.dataset.sku, column = this.dataset.column, value = this.value;
                fetch('/update-link', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    ...
                    if (res.success) {
                        ...
                        if (column === 'Stage' && value.toLowerCase() === 'mfrg progress') {
                            ...
                            fetch('/mfrg-progresses/insert', {
                                ...
                                body: JSON.stringify({ parent, sku: skuVal, order_qty, supplier, adv_date: advDate })
                            })
                            .then(r => r.json())
                            .then(r => {
                                if (r.success) {
                                    row.remove();
                                    const currentStage = document.getElementById('stage-filter').value.toLowerCase();
                                    if (currentStage && currentStage !== 'mfrg progress') {
                                        document.getElementById('stage-filter').dispatchEvent(new Event('change'));
                                    }
                                    if (groupedSkuData[parent]) {
                                        groupedSkuData[parent] = Object.fromEntries(
                                            Object.entries(groupedSkuData[parent]).filter(([key, val]) => val.SKU !== skuVal)
                                        );
                                    }
                                } else {
                                    alert('❌ Failed to insert: ' + r.message);
                                }
                            });
                        }
                    } else {
    ...
    $('#reviewForm').on('submit', function (e) {
        ...
        $.ajax({
            ...
            error: xhr => {
                alert('Error saving review: ' + (xhr.responseJSON?.message || 'Unknown error occurred'));
            }
        });
    });

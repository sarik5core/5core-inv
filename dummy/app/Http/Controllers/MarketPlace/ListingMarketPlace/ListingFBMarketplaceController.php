...
class ListingFBMarketplaceController extends Controller
{
    public function listingFBMarketplace(Request $request)
    {
        $mode = $request->query('mode');
        $demo = $request->query('demo');
        $percentage = Cache::remember('fbmarketplace_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingFBMarketplace', [
            'fbMarketplacePercentage' => $percentage,
            'mode' => $mode,
            'demo' => $demo,
        ]);
    }

    public function getViewListingFBMarketplaceData(Request $request)
    {
    ...
    public function saveStatus(Request $request)
    {
        ...
        $sku = $validated['sku'];
        $status = FBMarketplaceListingStatus::where('sku', $sku)->first();
        ...
        FBMarketplaceListingStatus::updateOrCreate(
            ['sku' => $validated['sku']],
            ['value' => $existing]
        );

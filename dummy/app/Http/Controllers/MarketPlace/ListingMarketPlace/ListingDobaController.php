...
class ListingDobaController extends Controller
{
    public function listingDoba(Request $request)
    {
        ...
        $percentage = Cache::remember('doba_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingDoba', [
    ...
    public function getViewListingDobaData(Request $request)
    {
        $productMasters = ProductMaster::whereNull('deleted_at')->get();
        $skus = $productMasters->pluck('sku')->unique()->toArray();

        $shopifyData = ShopifySku::whereIn('sku', $skus)->get()->keyBy('sku');
        $statusData = DobaListingStatus::whereIn('sku', $skus)->get()->keyBy('sku');
    ...
    public function saveStatus(Request $request)
    {
        ...
        $sku = $validated['sku'];
        $status = DobaListingStatus::where('sku', $sku)->first();
        ...
        DobaListingStatus::updateOrCreate(
            ['sku' => $validated['sku']],
            ['value' => $existing]
        );

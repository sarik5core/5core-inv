...
class ListingAutoDSController extends Controller
{
    public function listingAutoDS(Request $request)
    {
        ...
        $percentage = Cache::remember('autods_marketplace_percentage', now()->addDays(30), function () {
            return 100;
        });

        return view('market-places.listing-market-places.listingAutoDS', [
            'autoDSPercentage' => $percentage,
            'mode' => $mode,
            'demo' => $demo,
        ]);
    }

    public function getViewListingAutoDSData(Request $request)
    {
        $productMasters = ProductMaster::whereNull('deleted_at')->get();
        $skus = $productMasters->pluck('sku')->unique()->toArray();

        $shopifyData = ShopifySku::whereIn('sku', $skus)->get()->keyBy('sku');
        $statusData = AutoDSListingStatus::whereIn('sku', $skus)->get()->keyBy('sku');
    ...
    public function saveStatus(Request $request)
    {
        ...
        $sku = $validated['sku'];
        $status = AutoDSListingStatus::where('sku', $sku)->first();
        ...
        AutoDSListingStatus::updateOrCreate(
            ['sku' => $validated['sku']],
            ['value' => $existing]
        );
